...
<head>
    ...
    <title>Admin Dashboard - Yellow Captain Girl</title>
                    ...
                    <div class="card-body">
                        ...
                        <div class="data-tables">
                            <table id="dataTable" class="text-center">
                                ...
                                <tbody>
                                    <?php
                                    foreach ($mysqli->query('SELECT cg,COUNT(cg) FROM votesy GROUP BY cg') as $row) {
                                        echo "<tr>";
                                        $post = "Captain Girl";

                                        /* To check which name in the encription */
                                        foreach ($mysqli->query("SELECT enc,name FROM candidates WHERE house = 'yellow' AND post = 'cg'") as $row1) {
                                            if ($row['cg'] == $row1['enc']) {
                                                $cgname = $row1['name'];
                                                break;
                                            }
